...
def set_tasmota_power(mqtt_state: Dict[Any,Any], tasmota_bool_name: str, power_state: str, app):
    """
...
def parse_tasmota_sensor_payload(msg) -> TasmotaSensor:
    ...
    parsed_dict['Time'] = datetime.datetime.strptime(payload['Time'],'%Y-%m-%dT%H:%M:%S')
    for k in payload['ENERGY']:
        if k == "TotalStartTime":
            parsed_dict[k] = datetime.datetime.strptime(payload['ENERGY'][k],'%Y-%m-%dT%H:%M:%S')
        else:
...
def parse_aqara_payload(msg) -> AqaraSensor:
    ...
    for k in payload:
        if k == 'device':
            parsed_dict['friendlyName'] = payload['device']['friendlyName'].split('/')[-1]
            parsed_dict['value_name'] = "temp_" + payload['device']['friendlyName'].split('/')[-1]
        elif k == 'last_seen':
            parsed_dict[k] = datetime.datetime.strptime(payload[k], '%Y-%m-%dT%H:%M:%S%z')
        else:
...
def class_writer(msg):
    ...
    # else:
    #     print("NO MATCH")
    #     print(f"unmatched topic: {msg.topic}")
    #     print(f"unmatched message : {msg.payload}")
    return None 

def on_message_factory(app) -> Callable:
# def on_message_factory(mqtt_state: Dict[str,str]) -> Callable:
    ...
    def on_message_fn(client, userdata, message):
        ...
        try:
            ...
            if parsed_message is not None:
                ...
                mqtt_state['topic_states'][parsed_message.datatype][parsed_message.value_name] = parsed_message
...
def poll_topics(app):
    ...
    while True:
        ...
        # print(f"MQTT HEARTBEAT: {mqtt_state['mqtt_heartbeat']}")
